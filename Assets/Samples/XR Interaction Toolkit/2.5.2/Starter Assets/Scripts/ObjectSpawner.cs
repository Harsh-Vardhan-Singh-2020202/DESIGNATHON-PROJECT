...
namespace UnityEngine.XR.Interaction.Toolkit.Samples.StarterAssets
{
    ...
    public class ObjectSpawner : MonoBehaviour
    {
        ...
        // Add a dictionary to keep track of spawned objects for each prefab
        ...
        {
            // Check if the prefab is already spawned
            if (spawnedObjects.ContainsKey(m_ObjectPrefabs[m_SpawnOptionIndex]))
            {
                // If so, return false to prevent spawning new instance
                return false;
            }
            ...
            // Calculate the objectIndex before using it
            ...
            if (spawnedObjects.ContainsKey(m_ObjectPrefabs[m_SpawnOptionIndex]))
            {
                // If so, delete the existing spawned object
                Destroy(spawnedObjects[m_ObjectPrefabs[m_SpawnOptionIndex]]);
                spawnedObjects.Remove(m_ObjectPrefabs[m_SpawnOptionIndex]);
            }

            // Add the spawned object to the dictionary
            spawnedObjects.Add(m_ObjectPrefabs[m_SpawnOptionIndex], newObject);

            objectSpawned?.Invoke(newObject);
            return true;
        }

        public void DeleteSpawnedObject(GameObject objectToDelete)
        {
            if (spawnedObjects.ContainsKey(objectToDelete))
            {
                // Destroy the spawned object
                Destroy(spawnedObjects[objectToDelete]);
